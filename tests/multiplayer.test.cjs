...
test('third guest rejection and duplicate handshakes do not disturb admitted racers',()=>{
 ...
 s.host.receive(envelope(s,'hello',{characterId:'gray',engineVersion:1,rulesVersion:3},{senderId:'third'}));
 assert.equal(s.guest.phase,'racing');assert.equal(s.host.peerId,'g');
 s.host.receive(envelope(s,'hello',{characterId:'brown',engineVersion:1,rulesVersion:3}));
